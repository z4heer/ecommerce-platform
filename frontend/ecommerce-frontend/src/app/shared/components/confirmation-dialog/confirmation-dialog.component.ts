...
export class ConfirmationDialogComponent {
  ...
  readonly severityInput = input<'default' | 'info' | 'warning' | 'danger'>('default', {
    alias: 'severity',
  });
  ...
  readonly dialogSeverity = computed(
    () =>
      this.dialogData?.severity ??
      this.severityInput() ??
      DEFAULT_CONFIRMATION_DIALOG_CONFIG.severity,
  );
  readonly confirmLabel = computed(
    () =>
      this.dialogData?.confirmLabel ??
      this.confirmLabelInput() ??
      DEFAULT_CONFIRMATION_DIALOG_CONFIG.confirmLabel,
  );
  readonly cancelLabel = computed(
    () =>
      this.dialogData?.cancelLabel ??
      this.cancelLabelInput() ??
      DEFAULT_CONFIRMATION_DIALOG_CONFIG.cancelLabel,
  );
  readonly disableClose = computed(
    () =>
      this.dialogData?.disableClose ??
      this.disableCloseInput() ??
      DEFAULT_CONFIRMATION_DIALOG_CONFIG.disableClose,
  );
